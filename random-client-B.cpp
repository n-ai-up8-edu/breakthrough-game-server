...
// (join an existing game and play)             //
// ---------------------------------------------//
#include <cstdlib>
#include <iostream>
...
int main(int ac, char* av[]) {
  try {
    ...
    printf("identifying as rty player\n");
    boost::asio::write(s, boost::asio::buffer((char*)"id rty 111\n", 11));
    boost::asio::read_until(s, streamreply, '\n');
    std::getline(is, reply);

    printf("joining a game\n");
    boost::asio::write(s, boost::asio::buffer((char*)"join_game 0\n", 12));
    boost::asio::read_until(s, streamreply, '\n');
    std::getline(is, reply);
	...
	if(strncmp(reply.c_str(), "= you", 5) == 0) {
	  boost::asio::write(s, boost::asio::buffer((char*)"show_board\n", 12));
	  boost::asio::read_until(s, streamreply, '\n');
	  std::getline(is, reply);
